...
def run(job_input: IJobInput):
    ...
    # some definitions...
    filename_to_import = '' # <- !!! ENTER THE NAME OF THE FILE WITHIN THE QUOTATIONS !!!
    ...
    x_train, x_test, y_train, y_test = train_test_split(
       x,
       y,
       test_size= # <- !!! SET THE TEST SIZE TO 20% OF THE DATA !!! ,
       random_state=22
     )
    ...
    lasso = LassoCV(normalize= # <- !!! MAKE SURE TO NORMALIZE THE DATA - HINT: BOOLEANS,
                    random_state=22)
    lasso.fit(
        x_train,
        y_train
